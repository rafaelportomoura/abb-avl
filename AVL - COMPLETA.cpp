...
// Rotaciona Ã  esquerda a subarvore. Retorna a nova raiz da subarvore.
...
// Atualiza todas as ligacoes, para que ptNoh, fique no lugar deste nÃ³.
...
// Muda as ligacoes do pai e do filho para desligar o nó atual.
// Usado para remocao na situacao em que nao existem os dois filhos.
...
// Levanta excecao se nao encontrar a chave.
...
// Escreve o o conteudo de um nó no formato [altura:chave/valor].
...
// Escreve o conteudo da nivel a nivel
// Usado para conferir se a estrutra da arvore ta correta
void AVL::EscreverNivelANivel(ostream& saida) {
    queue<NohAVL*> filhos;
    NohAVL* fimDeNivel = new NohAVL(TChave(), TValor()); // nó especial para marcar fim de um nÃ­vel
    filhos.push(mPtRaiz);
...
// Remove da arvore que nao tiver uma dada chave.
void AVL::Remover(TChave chave) {
    if (mPtRaiz == NULL)
        throw runtime_error("Tentativa de remover elemento de uma arvore vazia.");
    else { 
